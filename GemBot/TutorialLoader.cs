...
public class Tutorial
{
    ...
    public static async Task<List<Tutorial>> LoadAll(ushort max)
    {
        ...
        string[] dat = JsonConvert.DeserializeObject<string[]>(await File.ReadAllTextAsync("Data/Tutorial/map.info")) ?? throw new InvalidOperationException();
        
        for (ushort i = 0; i < max; i++)
        {
            toReturn.Add(Load(await File.ReadAllTextAsync($"Data/Tutorial/{dat[i]}")));
        }
        return toReturn;
    }
